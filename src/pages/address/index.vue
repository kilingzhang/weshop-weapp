<template>
  <div class="address-nav">
    <div v-if="isAddAddress" class="add-address">
      <van-area
        @confirm="addAddress"
        @cancel="cancel"
        title="选择地址"
      ...
      <van-cell-group>
        ...
        <van-field
          ...
          label="手机号"
          placeholder="请输入手机号"
          :error-message="phoneError ? '手机号格式错误' : ''"
          border="false"
          :error="phoneError"
        />
        <van-field
          :value="addressDetail"
          @change="addressDetailInput"
          required
          label="详细地址"
          type="textarea"
          placeholder="请输入详细地址"
    ...
    <div v-else-if="!isAddAddress" class="address-items">
      <div class="address-item">
        <div class="card address" :key="i" v-for="(address,index) in getters.addresses"
          ...
          <div
            ...
            <div class="address-user" style="display:flex;flex-flow: row nowrap;justify-content: space-between;">
              <span>收货人：{{address.name}}</span> <span>{{address.phone}}</span>
            </div>
            <div class="address-address" style="display:flex;flex-flow: row nowrap;justify-content: space-between;">
              <span>收件地址：{{address.province}} {{address.city}} {{address.county}} {{address.address_detail}}</span>
            </div>
          </div>
  ...
  export default {
    ...
    methods: {
      ...
      setDefaultAddress(item) {
        ...
        Dialog.confirm({
          title: "设置默认",
          message: "确定设置当前为默认地址吗？"
        }).then(() => {
          toast("loading", "Loading...", 0);
          store.dispatch("SetDefaultAddress",
            {
              addressId: item.id,
              success: res => {
                toast("success", "设置成功~");
      ...
      refreshAddresses(callback) {
        store.dispatch("UpdateAddresses", {
          complete: res => {
            callback(res);
          }
        });
      ...
      addressInput(e) {
        this.address = e.mp.detail.values[2]["code"];
      },
      phoneInput(e) {
